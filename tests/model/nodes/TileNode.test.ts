...
describe('TileNode', () => {
  ...
  describe('getInDirection()', () => {
    it('should get sibling node within same horizontal TileNode', () => {
      ...
      const childFrameRect = <Frame>{ x: 0, y: 0, width: 10, height: 10 };
      const childSyncedFrame = new SyncedFrame(childFrameRect);
      const syncedNode = new TestSyncedFrameNode(childSyncedFrame);
      parentNode.addChild(syncedNode);

      const childFrameRect2 = <Frame>{ x: 10, y: 0, width: 10, height: 10 };
      const childSyncedFrame2 = new SyncedFrame(childFrameRect2);
      const syncedNode2 = new TestSyncedFrameNode(childSyncedFrame2);
      parentNode.addChild(syncedNode2);

      const childFrameRect3 = <Frame>{ x: 20, y: 0, width: 10, height: 10 };
      const childSyncedFrame3 = new SyncedFrame(childFrameRect3);
      const syncedNode3 = new TestSyncedFrameNode(childSyncedFrame3);
      parentNode.addChild(syncedNode3);

      expect(parentNode.getInDirection(syncedNode3, DIRECTIONS.LEFT)).toBe(syncedNode2);
      expect(parentNode.getInDirection(syncedNode, DIRECTIONS.RIGHT)).toBe(syncedNode2);
      expect(parentNode.getInDirection(syncedNode2, DIRECTIONS.LEFT)).toBe(syncedNode);
      expect(parentNode.getInDirection(syncedNode2, DIRECTIONS.RIGHT)).toBe(syncedNode3);
    });

    fit('should get an adjacent node in a seperate adjacent container', () => {
      const rootFrameRect = <Frame>{ x: 0, y: 0, width: 100, height: 100 };
      const rootSyncedFrame = new SyncedFrame(rootFrameRect);
      const rootNode = new TileNode(rootSyncedFrame);

      const parentFrameRect = <Frame>{ x: 0, y: 0, width: 50, height: 100 };
      const parentSyncedFrame = new SyncedFrame(parentFrameRect);
      const parentNode = new TileNode(parentSyncedFrame);
      rootNode.addChild(parentNode);

      const childFrameRect = <Frame>{ x: 0, y: 0, width: 25, height: 100 };
      const childSyncedFrame = new SyncedFrame(childFrameRect);
      const syncedNode = new TestSyncedFrameNode(childSyncedFrame);
      parentNode.addChild(syncedNode);

      const childFrameRect2 = <Frame>{ x: 25, y: 0, width: 50, height: 100 };
      const childSyncedFrame2 = new SyncedFrame(childFrameRect2);
      const syncedNode2 = new TestSyncedFrameNode(childSyncedFrame2);
      parentNode.addChild(syncedNode2);

      const parentFrameRect2 = <Frame>{ x: 50, y: 0, width: 50, height: 100 };
      const parentSyncedFrame2 = new SyncedFrame(parentFrameRect);
      const parentNode2 = new TileNode(parentSyncedFrame);
      rootNode.addChild(parentNode2);

      const childFrameRect3 = <Frame>{ x: 50, y: 0, width: 25, height: 100 };
      const childSyncedFrame3 = new SyncedFrame(childFrameRect);
      const syncedNode3 = new TestSyncedFrameNode(childSyncedFrame);
      parentNode2.addChild(syncedNode3);

      const childFrameRect4 = <Frame>{ x: 75, y: 0, width: 25, height: 100 };
      const childSyncedFrame4 = new SyncedFrame(childFrameRect2);
      const syncedNode4 = new TestSyncedFrameNode(childSyncedFrame2);
      parentNode2.addChild(syncedNode4);

      expect(parentNode.getInDirection(syncedNode2, DIRECTIONS.RIGHT)).toBe(parentNode2);
      expect(parentNode2.getInDirection(syncedNode3, DIRECTIONS.LEFT)).toBe(parentNode);
    });
  });
});
